...
```bash
docker tag frontend:v2 $ACR_NAME.azurecr.io/frontend:v2
...
1. [Azure Portal](https://portal.azure.com/) で既存のコンテナーアプリ `frontend` を開く
2. メニューの「アプリケーション」内「リビジョンとレプリカ」→「新しいリビジョンの作成」を選択
3. 「コンテナー」タブで`frontend`をクリックして編集
4. 表示されたウインドウ内の**イメージ タグ** を `v2` に変更

![image tag](../images/aca_image_tag_update.png)

5. 「作成」をクリックして新しいリビジョンを作成
